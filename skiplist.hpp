...
template<typename _Tkey, typename _Tval, typename _Tcpr, typename _Talloc, typename _Trand, int _Lvl_radio, int _Max_level, int _Mult>
...
//typedef _skiplist_traits<int,int,std::less<int>,std::allocator<std::pair<int,int> >,myrandom,32,0> _Ttraits;
...
struct _skiplist_node
{
	...
	typename _Ttraits::alloc_type::size_type lsize;
	_skiplist_level<_Ttraits> *levels;
};
...
public:
	...
	typedef typename _Ttraits::alloc_type allocator_type;
	typedef typename allocator_type::rebind<_skiplist_level<_Ttraits> >::other level_alloc;
	typedef typename allocator_type::rebind<_skiplist_node<_Ttraits> >::other node_alloc;
...
public:
	_skiplist_val()
	{
		...
		{
			head_->levels[i].forward = head_;
			head_->levels[i].backward = head_;
		}
		levelsize_ = 0;
		length_ = 0;
	}
...
public:
	iterator begin()
	{
		return iterator(head_->levels[0].forward);
	}
	const_iterator begin() const
	{
		return const_iterator(head_->levels[0].forward);
	}
	iterator end()
	{
	...
	iterator rbegin()
	{
		return iterator(head_->levels[0].backward);
	}
	const_iterator rbegin() const
	{
		return const_iterator(head_->levels[0].backward);
	}
	iterator rend()
	{
		return iterator(head_);
...
private:
	...
	{
		node_type* ret = nalloc_.allocate(1);
		ret->levels = lalloc_.allocate(lvl);
		ret->lsize = lvl;
		return ret;
	}
	void _dealloc_node(node_type* pnode)
	{
		lalloc_.deallocate(pnode->levels,pnode->lsize);
		nalloc_.deallocate(pnode,1);
	}
	void _create_node(node_type* pnode, const value_type& kv)
	{
		palloc_.construct(&pnode->kv, kv);
	}
	void _destroy_node(node_type* pnode)
	{
		palloc_.destroy(&pnode->kv);
	}
	iterator _find(const key_type& k)
	{
		...
		for(long i = levelsize_ - 1; i >= 0; --i)
			for(;;)
			{
				...
				{
					if(!kcpr_(k, _key(_level_forward(x,i))))
					{
						return iterator(_level_forward(x,i));
					}
					break;
				}
		...
		for(long i = levelsize_ - 1; i >= 0; --i)
			for(;;)
			{
				...
				else if(!kcpr_(_key(_level_forward(x,i)) , k))
				{
					if(!kcpr_(k, _key(_level_forward(x,i))))
					{
						return const_iterator(_level_forward(x,i));
					}
					break;
				}
				x = _level_forward(x,i);
...
template<typename _Tkey
	...
	, typename _Talloc = std::allocator<std::pair<_Tkey, _Tval> >
	, typename _Trand = _skiplist_random
	, int _Max_level = 32>
class skiplist
	: public _skiplist_val<_skiplist_traits<_Tkey, _Tval, _Tcpr, _Talloc, _Trand, 4, _Max_level, 0 >
{
public:
